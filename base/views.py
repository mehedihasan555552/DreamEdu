...
@api_view(['GET'])
def Blogs(request):
    filterset = BlogPostFilter(request.GET, queryset=Blog.objects.all().order_by('-created_at'))
    # Pagination
    resPerPage = 3
...
def getTeamMember(request):
    ...
    serializer = TeamMemberSerializer(teammember, many=True)

    return Response({'teammember': serializer.data})